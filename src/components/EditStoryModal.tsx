...
import { X, ImagePlus, ChevronDown, Trash2, RotateCcw, Sparkles, Loader2 } from 'lucide-react';
...
export const EditStoryModal: React.FC<EditStoryModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && scenario) {
      ...
      setDevelopment(scenario.development || '');
      setRestartChat(false);

      const cover = scenario.coverImage || '';
      setMediaUrl(cover);
      if (cover) {
        const isVid = cover.startsWith('data:video') || cover.includes('.mp4') || cover.includes('.webm');
        const isGif = cover.startsWith('data:image/gif') || cover.includes('.gif');
        setMediaType(isVid ? 'video' : isGif ? 'gif' : 'image');
        setMediaName('Portada actual');
      } else {
        setMediaType('image');
        setMediaName('');
      }
    ...
    reader.onload = async (event) => {
      ...
      try {
        const uploadRes = await fetch('/api/upload-media', {
          ...
          body: JSON.stringify({ media: result, scenarioId: scenario?.id || `story_${Date.now()}` })
        });
        if (uploadRes.ok) {
  ...
  const handleGenerateCardImage = async () => {
    ...
    try {
      let charName = scenario.characterName || "Valentina";
  ...
  const handleSave = () => {
    ...
    const updatedScenario: StoryScenario = {
      ...
      coverImage: mediaUrl || scenario.coverImage,
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold tracking-tight text-white">
            Editar Parámetros de la Historia
          </h2>
          <button 
            onClick={onClose}
            ...
            {!mediaUrl ? (
              ...
              <div className="relative rounded-xl overflow-hidden border border-pink-500/40 bg-black/40 flex items-center p-2 gap-3">
                ...
                <div className="flex-1 min-w-0 text-xs">
                  <div className="text-white font-medium truncate">{mediaName || 'Portada actual'}</div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    onClick={() => fileInputRef.current?.click()}
                    className="p-1.5 text-zinc-400 hover:text-white rounded-lg hover:bg-white/10 text-xs cursor-pointer"
